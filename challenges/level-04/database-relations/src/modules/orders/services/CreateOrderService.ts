...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const productsWithNoQuantitiesAvailable = products.filter(
      product =>
    ...
    if (productsWithNoQuantitiesAvailable.length) {
      throw new AppError(
        `The quantity ${productsWithNoQuantitiesAvailable[0].quantity} is not available for ${productsWithNoQuantitiesAvailable[0].id}.`,
      );
    }
